...
def test_rfm_calculation():
    ...
    df_with_rfm, rfm, cluster_summary = rfm_cluster.calculate_rfm_and_cluster(
        data, group_by_col='CustomerId', amount_col='Amount', datetime_col='TransactionStartTime'
    )
    ...
    expected_rfm = pd.DataFrame({
        ...
        'Frequency': [2, 1],
        'Monetary': [np.log1p(800), np.log1p(500)],  # log1p(1000 - 200), log1p(500)
    ...
    assert all(col in rfm.columns for col in ['CustomerId', 'Recency', 'Frequency', 'Monetary', 'Cluster', 'is_high_risk'])
...
def test_high_risk_assignment():
    ...
    data = pd.DataFrame({
        ...
        'TransactionStartTime': ['2018-11-15T02:18:49Z', '2018-11-16T02:19:08Z', 
                                '2018-11-14T02:44:21Z', '2018-11-17T03:32:55Z']
    })
    ...
    # Expected: C2 has highest recency (3 days), lowest frequency (1), lowest monetary
    assert rfm[rfm['CustomerId'] == 'C2']['is_high_risk'].iloc[0] == 1, "C2 should be high-risk"
    assert 'is_high_risk' in df_with_rfm.columns, "is_high_risk column missing in output DataFrame"
    assert df_with_rfm['is_high_risk'].isin([0, 1]).all(), "is_high_risk should be binary"
